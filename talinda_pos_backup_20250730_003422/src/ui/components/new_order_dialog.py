...
class NewOrderDialog(QDialog):
    ...
    def __init__(self, user: User, sale_controller: SaleController, parent=None, customer_name="", notes=""):
        super().__init__(parent)
        self.user = user
        self.sale_controller = sale_controller
        self.order_controller = OrderController()
        self.order = None
        self.prefilled_customer_name = customer_name
        self.prefilled_notes = notes
    ...
    def init_ui(self):
        ...
        # Header
        if self.prefilled_customer_name:
            header_label = QLabel(f"Save Order - {self.prefilled_customer_name}")
            header_label.setStyleSheet("color: #27ae60; margin-bottom: 10px;")
        else:
        ...
        # Show indicator if loaded from existing order
        if self.prefilled_customer_name:
            indicator_label = QLabel("📋 Loaded from existing order")
            indicator_label.setStyleSheet("color: #3498db; font-size: 12px; font-style: italic; margin-bottom: 10px;")
        ...
        self.customer_name_input.setText(self.prefilled_customer_name)
        ...
        self.notes_input.setPlainText(self.prefilled_notes)
