...
echo         self.update_status^("Installing required packages...", 30^)
...
echo                 self.update_status^(f"Installing {req}...", 30 + ^(len^(requirements^).index^(req^) * 5^)^)
...
echo             return False
echo.    
echo     def create_application_files^(self^):
echo         self.update_status^("Creating application files...", 60^)
echo         try:
...
echo.    
echo     # Import and show main window
...
echo             return True
echo         except Exception as e:
echo             messagebox.showerror^("Error", f"Failed to create files: {e}"^)
echo             return False
echo.    
...
echo                     return
echo                 if not self.create_application_files^(^):
echo                     return
...
python -m PyInstaller --clean talinda_pos_installer.spec
...
echo.
echo Installation builder completed!
pause 